import cv2, time, pyautogui  # Import required libraries
import mediapipe as mp       # MediaPipe for hand tracking


mp_hands=mp.solutions.hands
# Initialize MediaPipe Hands with one hand and confidence threshold
hands=mp_hands.Hands(max_num_hands=1,min_detection_confidence=0.7)
mp_drawing=mp.solutions.drawing_utils

# Scroll settings
SCROLL_SPEED=300      # Amount to scroll per gesture
SCROLL_DELAY=1        # Minimum delay between scrolls (seconds)
CAM_WIDTH,CAM_HEIGHT=640,480  # Camera resolution

def detect_gesture(landmarks,handedness):
    """
    Detects hand gesture based on finger positions.
    Returns 'scroll_up' for open palm, 'scroll_down' for fist, 'none' otherwise.
    """
    fingers=[]
    # List of finger tip landmarks (excluding thumb)
    tips=[mp_hands.HandLandmark.INDEX_FINGER_TIP,mp_hands.HandLandmark.MIDDLE_FINGER_TIP,
          mp_hands.HandLandmark.RING_FINGER_TIP,mp_hands.HandLandmark.PINKY_TIP]
    # Check if each finger is extended
    for tip in tips:
        if landmarks.landmark[tip].y<landmarks.landmark[tip-2].y:
            fingers.append(1)

    # Thumb detection
    thumb_tip=landmarks.landmark[mp_hands.HandLandmark.THUMB_TIP]
    thumb_ip=landmarks.landmark[mp_hands.HandLandmark.THUMB_IP]

    # For right hand, thumb_tip.x > thumb_ip.x means thumb is extended
    # For left hand, thumb_tip.x < thumb_ip.x means thumb is extended
    if (handedness=="Right" and thumb_tip.x>thumb_ip.x) or (handedness=="Left" and thumb_tip.x<thumb_ip.x):
        fingers.append(1)
    # If all fingers extended, scroll up; if none, scroll down; else, no action
    return "scroll_up" if sum(fingers)==5 else "scroll_down" if len(fingers)==0 else "none"

# Initialize webcam
cap=cv2.VideoCapture(0)
cap.set(3,CAM_WIDTH)
cap.set(4,CAM_HEIGHT)
last_scroll=p_time=0

# Display instructions
print("Gesture Control Active\nOpen Palm: Scroll Up\nFist: Scroll Down\nPress 'q' to Quit")

# Main loop
while cap.isOpened():
    success,img=cap.read()
    if not success:
        break

    # Convert image to RGB and flip horizontally for mirror effect
    img=cv2.flip(cv2.cvtColor(img,cv2.COLOR_BGR2RGB),1)
    results=hands.process(img)

    # Set default values for handedness and gesture
    handedness = "None"
    gesture = "none"

    # If hand landmarks detected
    if results.multi_hand_landmarks:
        for hand,handedness_info in zip(results.multi_hand_landmarks,results.multi_handedness):
            handedness=handedness_info.classification[0].label
            gesture=detect_gesture(hand,handedness)
            # Draw hand landmarks on image
            mp_drawing.draw_landmarks(img,hand,mp_hands.HAND_CONNECTIONS)

            # Scroll if enough time has passed since last scroll
            if (time.time()-last_scroll)>SCROLL_DELAY:
                if gesture=="scroll_up":
                    pyautogui.scroll(SCROLL_SPEED)
                elif gesture=="scroll_down":
                    pyautogui.scroll(-SCROLL_SPEED)
                    last_scroll=time.time()

    # Calculate and display FPS, hand type, and gesture
    fps=1/(time.time()-p_time) if (time.time()-p_time)>0 else 0
    p_time=time.time()
    cv2.putText(img,f"FPS: {int(fps)} | Hand: {handedness} | Gesture: {gesture}",(10,30),cv2.FONT_HERSHEY_SIMPLEX,0.7,(255,0,0),2)
    cv2.imshow("Gesture Control",cv2.cvtColor(img,cv2.COLOR_RGB2BGR))

    # Exit on 'q' key
    if cv2.waitKey(1) & 0xFF==ord('q'):
        break
# Release resources
cap.release()
cv2.destroyAllWindows()

